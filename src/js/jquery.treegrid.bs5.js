 ...
 * @license MIT
 * Change expanderExpandedClass and expanderCollapsedClass to FontAwesome5 icon by David Xu<david.xu.uts@163.com>
 */
(function($) {
    const methods = {
        /**
        ...
        initTree: function (options) {
            const settings = $.extend({}, this.treegrid.defaults, options);
        ...
        initExpander: function () {
            const $this = $(this);
            const cell = $this.find('td').get($this.treegrid('getSetting', 'treeColumn'));
            const tpl = $this.treegrid('getSetting', 'expanderTemplate');
            const expander = $this.treegrid('getSetting', 'getExpander').apply(this);
            if (expander) {
                expander.remove();
            }
        ...
        initIndent: function () {
            ...
            const tpl = $this.treegrid('getSetting', 'indentTemplate');
            const expander = $this.find('.treegrid-expander');
            const depth = $this.treegrid('getDepth');
        ...
        /**
         * Return true if this tree was never initialised
         *
         * @returns {Boolean}
         */
        isFirstInit: function () {
            const tree = $(this).treegrid('getTreeContainer');
        ...
        /**
         * Save state of the current node
         *
         * @returns {Node}
         */
        saveState: function () {
            ...
            if ($this.treegrid('getSetting', 'saveStateMethod') === 'cookie') {
                ...
                const stateArrayString = $.cookie($this.treegrid('getSetting', 'saveStateName')) || '';
                const stateArray = (stateArrayString === '' ? [] : stateArrayString.split(','));
        ...
        /**
         * Restore state of the current node.
         *
         * @returns {Node}
         */
        restoreState: function () {
            const $this = $(this);
            if ($this.treegrid('getSetting', 'saveStateMethod') === 'cookie') {
                const stateArray = $.cookie($this.treegrid('getSetting', 'saveStateName')).split(',');
        ...
        /**
         * Method return all root nodes of a tree.
         *
        ...
        /**
         * Method return parent id of node or null of root node
         *
         * @returns {String}
         */
        ...
        /**
         * Method return array of child nodes or null if a node is a leaf
         *
         * @returns {Object[]}
         */
        ...
        /**
         * Method return depth of a tree.
         *
         * This method is needed to calculate indent
         *
         * @returns {Number}
         */
        getDepth: function () {
        ...
        isLast: function () {
            if ($(this).treegrid('isNode')) {
                const parentNode = $(this).treegrid('getParentNode');
                if (parentNode === null) {
        ...
        /**
         * Return true if the node collapsed
         *
         * @returns {Boolean}
         */
        ...
        /**
         * Return true if at least one of the parent nodes is collapsed
         *
         * @returns {Boolean}
         */
        ...
        render: function () {
            return $(this).each(function () {
                const $this = $(this);
                //if parent collapsed when hidden
            ...
            return $(this).each(function () {
                ...
                if (expander) {
                    if (!$this.treegrid('isCollapsed')) {
                        ...
                        expander.html($this.treegrid('getSetting', 'expanderExpandedClass'));
                    } else {
                        ...
                        expander.html($this.treegrid('getSetting', 'expanderCollapsedClass'));
                    }
                } else {
    ...
    $.fn.treegrid.defaults = {
        ...
        expanderExpandedClass: '<i class="bi bi-caret-down-fill"></i>',
        expanderCollapsedClass: '<i class="bi bi-caret-right-fill"></i>',
        treeColumn: 0,
        getExpander: function() {
            return $(this).find('.treegrid-expander');
        },
        getNodeId: function() {
            const template = /treegrid-([A-Za-z0-9_-]+)/;
        ...
        getParentNodeId: function() {
            const template = /treegrid-parent-([A-Za-z0-9_-]+)/;
            if (template.test($(this).attr('class'))) {
                return template.exec($(this).attr('class'))[1];
            }
            return null;
        ...
        getNodeById: function(id, treegridContainer) {
            const templateClass = "treegrid-" + id;
            return treegridContainer.find('tr.' + templateClass);
        },
        getChildNodes: function(id, treegridContainer) {
            const templateClass = "treegrid-parent-" + id;
            return treegridContainer.find('tr.' + templateClass);
        },
        ...
        getRootNodes: function(treegridContainer) {
            const result = $.grep(treegridContainer.find('tr'), function (element) {
                const classNames = $(element).attr('class');
                const templateClass = /treegrid-([A-Za-z0-9_-]+)/;
                const templateParentClass = /treegrid-parent-([A-Za-z0-9_-]+)/;
